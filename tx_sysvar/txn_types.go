...
func (u *TransactionInfo) DecodeFromBlock(tx *pb.SubscribeUpdateTransactionInfo, slot uint64) {
	u.Signature = solana.SignatureFromBytes(tx.Signature)
	u.Transaction = u.decodeTransaction(tx.Transaction)
	u.Meta = u.decodeTransactionMeta(tx.Meta)
	u.Slot = slot
}
