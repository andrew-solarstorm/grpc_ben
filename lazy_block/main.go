...
func main() {
	...
	wsSvc := NewWSService(blockSvc)
	evtAgg := NewEventAggregator(wsSvc)
	blockSvc.evtAggSvc = evtAgg
	blockSvc.Subscribe(endpoint, token, commitment)
	decSvc := NewTransferDecService(blockSvc, evtAgg)
	txIngest := NewTxIngestService(blockSvc, decSvc)
