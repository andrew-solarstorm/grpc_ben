...
func NewTransferDecService(blckSvc *BlockService, evtAggSvc *EventAggregator) *Decoder {
	dec := &Decoder{
...
func (svc *Decoder) decodeTransfer(args *TxExtractArgs, ixAccounts []uint16, ixData []byte) (*Transfer, error) {
	...
	transfer := Transfer{
		...
		BlockTime:          svc.blckSvc.getBlockTime(args.Transaction.Slot),
